...
The full script can be downloaded from :download:`kuafu.py <../../../../examples/rendering/kuafu.py>`, :download:`kuafu_mat.py <../../../../examples/rendering/kuafu_mat.py>`, and :download:`kuafu_sensor.py <../../../../examples/rendering/kuafu_sensor.py>`
...
The `ActiveLightSensor` class has built-in sensor types such as `fakesense_j415`. We can also manually set all the sensor parameters by passing them to the class. Please check the API doc for details. The `sensor` behaves just like a mounted camera. You can `set_pose`, and `take_picture` on the sensor:
