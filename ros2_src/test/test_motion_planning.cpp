...
void test1(int argc, char *argv[]) {
  ...
  auto chair_loader = scene->createURDFLoader();
  chair_loader->scale = 0.1;
  auto chair = chair_loader->load("assets/148/mobility.urdf");
  chair->setRootPose({{0.3, 0, 0}, PxIdentity});

  // ROS2 specified class
  rclcpp::init(argc, argv);
  ros2::RobotManager::setResourcesDirectory(
      "/home/sim/project/sapien/python/py_ros2_package/ros2");
  ros2::SceneManager sceneManager(scene.get(), "scene1");
  auto descriptor =
      ros2::RobotDescriptor::fromROS("sapien_resources", "xarm6_description/urdf/xarm6.urdf",
                                     "xarm6_moveit_config/config/xarm6.srdf");

  auto loader = sceneManager.createRobotLoader();
  auto [robot, robotManager] = loader->loadRobotAndManager(*descriptor, "xarm6");

  robot->setRootPose({{0, 0, 0.0}, PxIdentity});
  ...
  scene->step();
  scene->updateRender();
  for (auto &link : chair->getBaseLinks()) {
    planner->updateCollisionObjects(link);
  }
  chair->setRootPose({{0.6, 0, 0}, PxIdentity});
  for (auto &link : chair->getBaseLinks()) {
    planner->updateCollisionObjects(link);
  }
  ...
  auto activeJoint = robot->getActiveJoints();
  while (!controller.shouldQuit()) {
    ...
    if (step == 500) {
      ...
      for (int j = 0; j < plan.position.rows(); ++j) {
        auto now = sceneManager.now();
        rclcpp::Time end = now + rclcpp::Duration(rcl_duration_value_t(plan.duration[j] * 1e9));
        auto target = plan.position.row(j);
        for (int k = 0; k < 6; ++k) {
          activeJoint[k]->setDriveTarget(target[k]);
        }
        while (now < end) {
          now = sceneManager.now();
          scene->step();
          scene->updateRender();
          controller.render();
          step++;
