...
private:
  std::string mName;
  PxArticulationJointReducedCoordinate *getPxJoint() const;
  std::vector<PxArticulationAxis::Enum> mAxes;
...
template <class Archive> void PhysxArticulationLinkComponent::load(Archive &ar) {
  ...
  if (!mParent) {
    ...
    for (int i = 0; i < 6; ++i) {
      pxjoint->setMotion(axes[i], PxArticulationMotion::Enum(jointMotion[i]));
      pxjoint->setLimitParams(axes[i], {jointLimit[i][0], jointLimit[i][1]});
      pxjoint->setDriveParams(
          axes[i], PxArticulationDrive(jointDrive[i][0], jointDrive[i][1], jointDrive[i][2],
                                       PxArticulationDriveType::Enum(jointDriveType[i])));
...
CEREAL_REGISTER_TYPE(sapien::component::PhysxArticulationLinkComponent);
CEREAL_REGISTER_POLYMORPHIC_RELATION(sapien::component::PhysxRigidBodyComponent,
                                     sapien::component::PhysxArticulationLinkComponent);
