...
RUN mkdir -p /workspace && cd /workspace && wget https://developer.download.nvidia.com/compute/cuda/12.8.0/local_installers/cuda_12.8.0_570.86.10_linux_sbsa.run  && \
    sh cuda_12.8.0_570.86.10_linux_sbsa.run --silent --toolkit --toolkitpath=/workspace/cuda --override && rm -f cuda_12.8.0_570.86.10_linux_sbsa.run
