...
class MOVOFreeBaseEnv(MOVOEnv):
    def __init__(self):
        """
        Sapien Kinova MOVO Free Base base class.
        If you want to use it with sapien object environment, do not use __init__ but _init_robot" (most case)
        If you just want to load the robot only, you should consider use __init__ but not _init_robot
        """
        BaseRobotEnv.__init__(self)
    ...
    def _load_controller_parameters(self):
        ...
        self.root_theta = 0
        self.root_pos = np.array([0, 0], dtype=np.float)
        self.robot.set_root_pose([0, 0, 0])
        self.init_qpos = [0, 0, 0, 0.25, -1.381, 0, 0.05, -0.9512, 0.387, 0.608, 2.486, 1.05, -1.16, 0, 0, 0]

        # Tune PD controller
        self.robot.set_pd(20000, 5000, 2000, np.arange(4))
        self.robot.set_pd(2000, 300, 300, np.array([1, 3, 5, 6, 7, 8, 9]) + 3)
        self.robot.set_pd(500, 100, 300, np.array([2, 4]) + 3)
    ...
    def _load_ros_controller(self) -> None:
        ...
        self._rotation_joint = ["y_rotation_joint"]
