...
glm::mat4 SVulkan2Camera::getCameraMatrix() const {
  if (mCamera->isOrthographic()) {
    throw std::runtime_error("Orthographic camera matrix is not supported");
  }
...
void SVulkan2Camera::changeModeToOrthographic(float scaling) {
  mCamera->setOrthographicParameters(mCamera->getNear(), mCamera->getFar(), mCamera->getAspect(),
                                     scaling);
}

void SVulkan2Camera::changeModeToPerspective(float fovy) {
  mCamera->setPerspectiveParameters(mCamera->getNear(), mCamera->getFar(), fovy,
                                    mCamera->getAspect());
}
