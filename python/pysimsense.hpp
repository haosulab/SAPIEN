...
public:
  DepthSensorEnginePython(uint32_t _rows, uint32_t _cols, float _focalLen, float _baselineLen,
                          ...
                          uint8_t _censusHeight, uint32_t _maxDisp, uint8_t _bfWidth,
                          uint8_t _bfHeight, uint8_t _p1, uint8_t _p2, uint8_t _uniqRatio,
                          uint8_t _lrMaxDiff, uint8_t _mfSize, py::array_t<float> mapLx,
                          py::array_t<float> mapLy, py::array_t<float> mapRx,
                          py::array_t<float> mapRy, float _mainFx, float _mainFy, float _mainSkew,
                          float _mainCx, float _mainCy)
      : DepthSensorEngine(_rows, _cols, _focalLen, _baselineLen, _minDepth, _maxDepth,
                          infraredNoiseSeed, _speckleShape, _speckleScale, _gaussianMu,
                          _gaussianSigma, _rectified, _censusWidth, _censusHeight, _maxDisp,
                          _bfWidth, _bfHeight, _p1, _p2, _uniqRatio, _lrMaxDiff, _mfSize,
                          ndarray2Mat2d<float>(mapLx), ndarray2Mat2d<float>(mapLy),
                          ndarray2Mat2d<float>(mapRx), ndarray2Mat2d<float>(mapRy), _mainFx,
                          _mainFy, _mainSkew, _mainCx, _mainCy) {}
...
void buildSimsense(py::module &parent) {
  ...
  PySimsense.def(
      py::init<uint32_t, uint32_t, float, float, float, float, uint64_t, float, float, float,
               float, bool, uint8_t, uint8_t, uint32_t, uint8_t, uint8_t, uint8_t, uint8_t,
               uint8_t, uint8_t, uint8_t, py::array_t<float>, py::array_t<float>,
               py::array_t<float>, py::array_t<float>, float, float, float, float, float>());
  PySimsense.def(
  ...
  PySimsense.def("compute",
                 py::overload_cast<py::capsule, py::capsule>(&DepthSensorEnginePython::compute));
