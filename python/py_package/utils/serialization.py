...
class SerializedScene:
    ...
    def __init__(self, scene: Scene):
        ...
        self.serialized_default_material = self.SerializedMaterial(scene.default_physical_material)
        ...
        for actor in scene.get_all_actors(): # Save used physical materials of actors
            for collision_shape in actor.get_collision_shapes():
                ...
                if material_id != default_material_id and material_id not in self.serialized_materials:
                    self.serialized_materials[material_id] = self.SerializedMaterial(material)

        for articulation in scene.get_all_articulations(): # Save used physical materials of articulations
    ...
    def deserialize(self, engine: Engine) -> Scene:
        ...
        # Default physical material
        scene.default_physical_material = self.serialized_default_material.deserialize(engine)
        ...
        for actor_id, actor_data in self.serialized_actors.items():
            ...
            if actor_data['type'] == 'dynamic':
                ...
                actor.set_damping(actor_data['linear_damping'], actor_data['angular_damping'])
    ...
    class SerializedMaterial:
        ...
        def deserialize(self, engine: Engine) -> PhysicalMaterial:
            material = engine.create_physical_material(self.static_friction, self.dynamic_friction, self.restitution)
            return material
    ...
    class SerializedGroundBuilder:
        ...
        def deserialize(self, scene:Scene, materials: dict) -> 'SerializedScene.GroundBuilder':
            return SerializedScene.GroundBuilder(scene, self.altitude, materials[self.material_id])
    ...
    class SerializedLinkBuilder(SerializedActorBuilder):
        ...
        def deserialize(self, art_builder: ArticulationBuilder, materials: dict) -> LinkBuilder:
    ...
    class SerializedArticulationBuilder():
        def __init__(self, builder: ArticulationBuilder):
            ...
            for link_builder in builder.get_link_builders():
                self.serialized_link_builders.append(SerializedScene.SerializedLinkBuilder(link_builder))
    ...
    class SerializedDrive():
        ...
        def deserialize(self, scene: Scene, actors: dict) -> Drive:
            ...
            if self.dof_states[0] == 'limited' and \
                self.dof_states[1] == 'limited' and \
                self.dof_states[2] == 'limited':
