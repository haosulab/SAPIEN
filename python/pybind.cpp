...
PYBIND11_MODULE(sapyen, m) {
  py::class_<Simulation>(m, "Simulation")
      ...
      .def("get_render_name_dict", [](Simulation &sim) { return sim.mRenderId2VisualName; },
  ...
  py::class_<Renderer::MountedCamera, Optifuser::CameraSpec, Renderer::ICamera>(m,
      ...
      .def("take_raytraced_picture",
           [](Renderer::MountedCamera &cam, uint32_t samplesPerPixel, uint32_t reflectionCount) {
             return py::array_t<float>(
                 {static_cast<int>(cam.getHeight()), static_cast<int>(cam.getWidth()), 4},
                 cam.takeRaytracedPicture(samplesPerPixel, reflectionCount).data());
           },
           py::arg("samples_per_pixel") = 65536, py::arg("reflection_count") = 4)
#endif
      ;
  ...
  py::class_<ArticulationBuilder>(m, "ArticulationBuilder")
      ...
      .def("add_sphere_shape_to_link", &ArticulationBuilder::addSphereShapeToLink, py::arg("link"),
           ...
           py::arg("material") = nullptr)
      .def("add_convex_obj_shape_to_link",
