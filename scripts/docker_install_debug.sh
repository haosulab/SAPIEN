...
docker run -v "$(pwd)":/workspace/SAPIEN -it --rm \
       ...
       fxiangucsd/sapien-build-env:3.9 bash -c "export CMAKE_BUILD_PARALLEL_LEVEL=${CMAKE_BUILD_PARALLEL_LEVEL} && cd /workspace/SAPIEN && ./scripts/build.sh ${PYTHON_VERSION} --debug"
