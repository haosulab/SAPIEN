...
TEST(SapienRenderTexture, CreateMaterial) {
  auto meshfile = std::filesystem::path(__FILE__).parent_path().parent_path().parent_path() /
                  "assets" / "test_texture.png";
  auto texture = std::make_shared<SapienRenderTexture2D>(meshfile.string(), 3,
                                                       SapienRenderTexture2D::FilterMode::eNEAREST,
...
TEST(SapienRenderTexture, SerializeMaterial) {
  auto meshfile = std::filesystem::path(__FILE__).parent_path().parent_path().parent_path() /
                  "assets" / "test_texture.png";
  auto texture = std::make_shared<SapienRenderTexture2D>(meshfile.string(), 3,
                                                       SapienRenderTexture2D::FilterMode::eNEAREST,
  ...
  ASSERT_TRUE(material->getNormalTexture());
  EXPECT_EQ(material->getNormalTexture()->getWidth(), 64);
  EXPECT_EQ(material->getNormalTexture()->getHeight(), 32);
}
