...
def main():
    ...
    depth_cpu = sensor.get_depth()
    print("Runtime for get_depth: ", time.process_time() - start)
    sub = fig.add_subplot(1, 2, 1)
    sub.imshow(depth_cpu)

    sensor.clear_cache()
    
    # GPU implementation
    start = time.process_time()
    depth_gpu = sensor.get_depth_gpu()
    print("Runtime for get_depth_gpu: ", time.process_time() - start)
    sub = fig.add_subplot(1, 2, 2)
    sub.imshow(depth_gpu)
    ...
    sensor.close_gpu() # release GPU resources
